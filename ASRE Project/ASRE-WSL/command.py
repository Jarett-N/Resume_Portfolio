...
class CommandModule:
    ...
    def sendAction(self, action_path: str):
        ...
        try:
            ...
            # Turn off AutonomousLife and Awareness if present
            try:
                self._session.service("ALAutonomousLife").setState("disabled")
            except:
                pass
            try:
                self._session.service("ALBasicAwareness").stopAwareness()
            except:
                pass
        ...
        if audio_cfg:
            self._schedule_audio_or_tts(audio_cfg, start_time)
    ...
    # Audio Scheduler
    def _schedule_audio_or_tts(self, audio_cfg: Dict[str, Any], start_time: float):
        # Play an audio file if it exists, otherwise fall back to TTS if text is provided.
        file_path = audio_cfg.get("externalFile")
        ...
        def play_audio():
            ...
            try:
                # If file_path looks like a URL (http or https)
                if file_path and file_path.startswith(("http://", "https://")):
                    print(f"[ASRE] Streaming audio from URL: {file_path}")
                    self._audio.playFile(file_path)
                    logging.info(f"Audio streamed from URL: {file_path}")

                # If file_path is a local file path and exists
                # THIS DOES NOT WORK, REMAKE WITH REMOTE BUFFER
